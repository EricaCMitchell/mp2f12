...
void DiskMP2F12::convert_C(einsums::Tensor<double,2> *C, OrbitalSpace bs, const int& dim1, const int& dim2)
{
#pragma omp parallel for collapse(2) num_threads(nthreads_)
    for (int p = 0; p < dim1; p++) {
        for (int q = 0; q < dim2; q++) {
...
void DiskMP2F12::set_ERI(einsums::TensorView<double, 3>& ERI_Slice, einsums::Tensor<double, 3> *Slice)
{
    auto naux_= (*Slice).dim(0);
    auto dim1 = (*Slice).dim(1);
    auto dim2 = (*Slice).dim(2);
...
void DiskMP2F12::two_body_ao_computer(const std::string& int_type, einsums::Tensor<double, 4> *GAO,
                                  std::shared_ptr<BasisSet> bs1, std::shared_ptr<BasisSet> bs2,
                                  std::shared_ptr<BasisSet> bs3, std::shared_ptr<BasisSet> bs4)
{
    using namespace einsums;
...
void DiskMP2F12::three_index_ao_computer(const std::string& int_type, einsums::Tensor<double, 3> *Bpq,
                                     std::shared_ptr<BasisSet> bs1, std::shared_ptr<BasisSet> bs2)
{
    std::shared_ptr<BasisSet> zero(BasisSet::zero_ao_basis_set());
    std::shared_ptr<IntegralFactory> intf(new IntegralFactory(DFBS_, zero, bs1, bs2));
...
void DiskMP2F12::form_metric_ints(einsums::Tensor<double, 3> *DF_ERI, bool is_fock)
{
    using namespace einsums;
    using namespace tensor_algebra;
    using namespace tensor_algebra::index;
    ...
    for (int idx = 0; idx < (order.size()/2); idx++) {
        ...
        {
            // C2
            auto BpQ = std::make_unique<Tensor<double, 3>>("BpQ", naux_, nbf1, nmo2);
            einsum(Indices{B, p, Q}, BpQ.get(), Indices{B, p, q}, Bpq, Indices{q, Q}, C2);
            Bpq.reset();
            C2.reset();

            // C1
            auto BQp = std::make_unique<Tensor<double, 3>>("BQp", naux_, nmo2, nbf1);
            sort(Indices{B, Q, p}, BQp.get(), Indices{B, p, Q}, BpQ);
            BpQ.reset();
            auto BQP = std::make_unique<Tensor<double, 3>>("BQP", naux_, nmo2, nmo1);
            einsum(Indices{B, Q, P}, BQP.get(), Indices{B, Q, p}, BQp, Indices{p, P}, C1);
            BQp.reset();
            C1.reset();
            sort(Indices{B, P, Q}, BPQ.get(), Indices{B, Q, P}, BQP);
        }

        // Contract three-center integral with full inverse metric
        auto APQ = std::make_unique<Tensor<double, 3>>("APQ", naux_, nmo1, nmo2);
        {
            auto metric = std::make_shared<FittingMetric>(DFBS_, true);
            metric->form_full_eig_inverse(1.0e-12);
...
void DiskMP2F12::form_oper_ints(const std::string& int_type, einsums::Tensor<double, 3> *DF_ERI)
{
    using namespace einsums;
    using namespace tensor_algebra;
    using namespace tensor_algebra::index;
...
void DiskMP2F12::form_oper_ints(const std::string& int_type, einsums::Tensor<double, 2> *DF_ERI)
{
    using namespace einsums;
    using namespace tensor_algebra;
    using namespace tensor_algebra::index;
