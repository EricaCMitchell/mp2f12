...
#pragma omp parallel for collapse(4)
	...
	if (nbf4 != nbf1 && nbf4 != nbf2 && nbf4 != nbf3) {
...
void FF_mat(einsums::Tensor<double, 4> *FF, einsums::Tensor<double, 4> *F2, einsums::Tensor<double, 4> *F,
    ...
    timer::push("Allocations");
    TensorView<double, 4> F2_1ooo{(*F2), Dim<4>{nri, nocc, nocc, nocc}, Offset<4>{0,0,0,0}};
    ...
    einsum(Indices{k, l, m, n}, &klmn_F2, Indices{I, l, m, n}, F2_1ooo, Indices{k, I}, fk_o1);
    timer::pop();
...
SharedWavefunction mp2f12(SharedWavefunction ref_wfn, Options& options)
{
    ...
    auto F2 = std::make_unique<Tensor<double, 4>>("MO F12_Squared Tensor", nri, nobs, nobs, nobs);
    ...
    if (READ_INTS){
         ...
         read_ints_disk(F2.get(), nri, nobs, nobs, nobs);
         ...
         write_ints_disk(F2.get(), nri, nobs, nobs, nobs);
